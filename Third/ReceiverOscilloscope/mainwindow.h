...
private:
    ...
    QByteArray socketBuf; //Буфер сокета
    ...
    std::function<void(double,double)> painter = ([this](double x, double y){Analog(x,y);}); //Указатель C++11 на функцию отрисовки(Analog, Electic)
    ...
    std::thread *paintThread; //Поток для отрисовки графика
